document.addEventListener('DOMContentLoaded', () => {
    ...
    const partnerCardsSection = document.getElementById('partner-cards-section');
    const partnerNameEl = document.getElementById('partner-name');
    const partnerCardsEl = document.getElementById('partner-cards');
    ...
    const maoDeOnzeOverlay = document.getElementById('mao-de-onze-overlay');
    const maoDeOnzeButtons = document.querySelector('.mao-de-onze-buttons');
    ...
    socket.on('update_state', (state) => {
        ...
        tableCardsEl.innerHTML = state.mesa.length > 0 
            ? state.mesa.map(item => `<div class="card-image"><img src="/static/images/cards/${item.card_filename}" alt="${item.card_filename}"></div>`).join('') 
            : 'Aguardando jogadas...';
        ...
        if (state.gameWinner) {
            ...
            newRoundButton.style.display = 'none';
        } else if (state.maoDeOnzeState === 'pending') {
            gameMessage.textContent = 'Mão de Onze! Aguardando decisão...';
        } else {
        ...
        if (state.partnerCards && state.partnerCards.length > 0) {
            ...
            partnerCardsEl.innerHTML = state.partnerCards.map(cardFile => 
                `<div class="card-image"><img src="/static/images/cards/${cardFile}" alt="${cardFile}"></div>`
            ).join('');
            partnerCardsSection.style.display = 'block';
        } else {
            partnerCardsSection.style.display = 'none';
        }

        playHiddenOption.style.display = (state.gameStarted && state.mao > 1 && state.isMyTurn) ? 'block' : 'none';

        const canCallTruco = state.isMyTeamsTurn && state.trucoState === 'none' && state.mao > 1 && !state.roundOver && state.maoDeOnzeState === 'none';
        trucoButton.style.display = canCallTruco ? 'inline-block' : 'none';

        const shouldRespondTruco = state.trucoState !== 'none' && state.trucoAskerTeam !== state.myTeam;
        if (shouldRespondTruco) {
        ...
        const isMyTeamOnze = (state.myTeam === 1 && state.placar.time1 === 11) || (state.myTeam === 2 && state.placar.time2 === 11);
        maoDeOnzeOverlay.style.display = state.maoDeOnzeState === 'pending' && isMyTeamOnze ? 'flex' : 'none';
    });
    ...
    trucoResponseButtons.addEventListener('click', (e) => {
        if (e.target.tagName === 'BUTTON') {
            ...
            if(response === 'raise') socket.emit('call_truco', { room: currentRoom });
            else socket.emit('respond_truco', { room: currentRoom, response });
            trucoResponseOverlay.style.display = 'none';
        }
    });

    maoDeOnzeButtons.addEventListener('click', (e) => {
        if (e.target.tagName === 'BUTTON') {
            const response = e.target.dataset.response;
            socket.emit('respond_mao_de_onze', { room: currentRoom, response });
            maoDeOnzeOverlay.style.display = 'none';
        }
    });
});
