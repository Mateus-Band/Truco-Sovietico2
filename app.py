...
CARD_DATA = {
    'zap.png': {'value': 11, 'name': 'Zap'}, 'copas.png': {'value': 10, 'name': 'Copas'},
    'espadilha.png': {'value': 9, 'name': 'Espadilha'}, 'ouros.png': {'value': 8, 'name': 'Ouros'},
    'coringa.png': {'value': 7, 'name': 'Coringa'}, 'porcao.png': {'value': 0, 'name': 'Porcão'},
...
FULL_DECK = [k for k in CARD_DATA.keys() if k != 'virada.png']
...
def create_new_game_state():
    return {
        ...
        "truco_asker_team": None, "mao_de_onze_state": "none" # none, pending, running
    }
...
def get_views_for_all_players(room_name):
    ...
    with games_lock:
        ...
        for sid in player_sids:
            ...
            partner_cards = []
            if state['mao_de_onze_state'] != 'none':
                if (state['team1_score'] == 11 and player_team == 1) or \
                   (state['team2_score'] == 11 and player_team == 2) or \
                   (state['team1_score'] == 11 and state['team2_score'] == 11):
                    partner_cards = partner_info['cards'] if partner_info else []
            ...
            views[sid] = {
                ...
                "mesa": [{"player": p["name"], "card_filename": item["card_filename"]} for item in state["table"] for p in state["player_slots"] if p and p['sid'] == item['sid']],
                "handsHistory": [[{"player": p["name"], "card_filename": card_item["card_filename"]} for card_item in hand for p in state["player_slots"] if p and p['sid'] == card_item['sid']] for hand in state["hands_history"]],
                ...
                "maoDeOnzeState": state['mao_de_onze_state']
            }
    return views
...
@socketio.on('respond_mao_de_onze')
def on_respond_mao_de_onze(data):
    room, response = data['room'], data['response']
    with games_lock:
        state = games.get(room)
        if state['mao_de_onze_state'] != 'pending': return
        
        if response == 'accept':
            state['mao_de_onze_state'] = 'running'
            state['round_value'] = 3
        elif response == 'refuse':
            winner_team = 1 if state['team2_score'] == 11 else 2
            if winner_team == 1: state['team1_score'] += 1
            else: state['team2_score'] += 1
            state['round_over'] = True
        check_for_game_winner(state)
    broadcast_state(room)

# (O resto do app.py, a partir de 'disconnect', permanece igual, exceto por start_new_round e call_truco)
...
def on_call_truco(data):
    ...
    with games_lock:
        state = games.get(room)
        if not state['game_started'] or state['hand_of_round'] == 1 or state['mao_de_onze_state'] != 'none': return
        player_slot_idx = next((i for i, p in enumerate(state["player_slots"]) if p and p['sid'] == sid), -1)
        player_team = 1 if player_slot_idx in [0, 2] else 2
...
def start_new_round(room):
    with games_lock:
        state = games[room]
        state.update({"game_started": True, "round_over": False, "round_winner_team": None, "team1_hand_wins": 0, "team2_hand_wins": 0, "tied_hands": 0, "first_hand_winner_team": None, "round_value": 1, "truco_state": "none", "hands_history": [], "mao_de_onze_state": "none"})
        ...
        # Mão de Onze Check
        team1_is_onze = state['team1_score'] == 11
        team2_is_onze = state['team2_score'] == 11
        if team1_is_onze and team2_is_onze:
            state['mao_de_onze_state'] = 'running' # Mão de ferro, obrigatório jogar
            state['round_value'] = 3
        elif team1_is_onze or team2_is_onze:
            state['mao_de_onze_state'] = 'pending' # Aguardando decisão
            state['game_started'] = False # Pausa o jogo
        ...
        if state['mao_de_onze_state'] != 'pending':
